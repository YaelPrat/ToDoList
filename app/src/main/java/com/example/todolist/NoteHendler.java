...
public class NoteHendler extends DatabaseHelper {
    public NoteHendler(Context context){
        super(context);
    }
    ...
    public boolean create(Note note){
        ...
        boolean isOk = db.insert("Note",null,values) > 0;
        db.close();
        return  isOk;
    }
    ...
    public ArrayList<Note> readNotes(){
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(sqlQuery, null); //instead off nul we can put the select query
    ...
    public boolean update(Note note){
        ...
        values.put("description",note.getDescription());
        SQLiteDatabase db = this.getWritableDatabase();
        boolean isOK = db.update("Note",values,"id='"+note.getId()+"'",null) >0;
        db.close();
        return isOK;
    }
